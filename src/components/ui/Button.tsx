...
const Button = React.forwardRef<HTMLButtonElement, ButtonProps>(
  ({ className, variant = "default", size = "default", asChild = false, children, style, ...props }, ref) => {
    ...
    if (variant === "outline") {
      ...
      return (
        <button
          className={cn(`content-stretch flex items-center justify-center relative shrink-0 cursor-pointer ${padding}`, className)}
          ref={ref}
          style={style}
          {...props}
        >
          <div aria-hidden="true" className={`absolute border border-[#e9f4f9] border-solid inset-0 pointer-events-none ${borderRounding}`} />
          <p className={`[word-break:break-word] font-['Chivo_Mono_Medium:Regular',sans-serif] leading-[1.3] not-italic relative shrink-0 uppercase whitespace-nowrap ${textClass}`}>
            {children}
          </p>
        </button>
      )
